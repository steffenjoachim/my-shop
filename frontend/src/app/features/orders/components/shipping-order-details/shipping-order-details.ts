    ...
    <div class="max-w-4xl mx-auto p-4">
      ...
      <div class="bg-gray-50 border p-4 rounded-lg text-sm mb-6">
        <div class="mb-3">
          ...
          <!-- Status speichern: einheitliches Design (enabled: weiß mit blauem Rand, hover voll blau; disabled: dunkelgrau) -->
          <button
            (click)="updateStatus()"
            ...
            "
          >
            @if (updating) { ⏳ Speichern... } @else { 💾 Status speichern }
          </button>
        </div>
      ...
      <div class="bg-white border p-4 rounded-lg text-sm mb-6">
        ...
        <div class="mt-4 flex gap-3">
          ...
          <button
            ...
            "
          >
            @if (updatingShipping) { ⏳ Speichern... } @else { 💾 Speichern }
          </button>

          @if (selectedStatus !== 'ready_to_ship') {
          <p class="text-xs text-gray-500 self-center">
            Versandinfos können erst gespeichert werden, wenn der Status auf
...
export class ShippingOrderDetails implements OnInit, OnDestroy {
  ...
  updateStatus(): void {
    ...
    if (this.requiresShippingDetails()) {
      if (!this.selectedCarrier) {
        this.showStatusMessage('Bitte einen Versanddienst auswählen.', 'error');
        return;
      }
  ...
  // Neuer separater Speichern-Button für Versandinfos
  saveShippingInfo(): void {
    if (!this.order) return;

    if (this.selectedStatus !== 'ready_to_ship') {
      this.showStatusMessage(
        'Versandinformationen können erst gespeichert werden, wenn der Status auf "Versandbereit" gesetzt ist.',
        'error'
      );
      return;
    }
