        ...
        <div class="max-w-4xl mx-auto p-6">
          ...
          <section class="mb-6">
            ...
            <div class="space-y-3">
              ...
              <label
                class="flex gap-3 items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50"
              >
                <input
                  type="radio"
                  name="returnItem"
                ...
                <div class="flex-1">
                  ...
                  <div class="text-xs text-gray-500 mt-1">
                    @for (v of (item.variation_details?.attributes || []); track
                    v.id) {
                    <span class="inline-block mr-2"
                      >{{ v.attribute_type }}: {{ v.value }}</span
                    >
                    }
                  </div>
                  }
          ...
          <section class="mb-6">
            ...
            >
              ...
              <option value="falscher_artikel">
                Falscher Artikel geliefert
              </option>
              <option value="falsche_groesse">
                Falsche Größe / passt nicht
              </option>
...
export class RetourRequest implements OnInit {
        ...
        next: (res) => {
          ...
          res.items = (res.items || []).map((it: any) => ({
            ...
            variation_details:
              ...
              Array.isArray(it.variation_details.attributes)
                ? it.variation_details
                : { attributes: [] },
          }));
          this.order.set(res);
  ...
  canSubmit(): boolean {
    ...
    if (this.selectedReason === 'sonstiges' && !this.otherReason.trim())
      return false;
    return true;
  }
  ...
  submit() {
    ...
    if (this.selectedReason === 'sonstiges')
      payload.other_reason = this.otherReason.trim();

    this.submitting.set(true);
    this.http
      .post(`${environment.apiBaseUrl}orders/${id}/request_return/`, payload, {
        withCredentials: true,
      })
      .subscribe({
        next: () => {
        ...
        error: (err) => {
          ...
          this.submitting.set(false);
          this.showAlertMsg(
            'Retour-Anfrage konnte nicht gesendet werden.',
            'error'
          );
        },
      });
